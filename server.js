...
const rateLimiter = (req, res, next) => {
  ...
  if (attempts && attempts.count >= 5 && Date.now() - attempts.lastAttempt < 15 * 60 * 1000) {
    const minutesLeft = Math.ceil((15 * 60 * 1000 - (Date.now() - attempts.lastAttempt)) / (60 * 1000));
    return res.status(429).json({ error: `Too many failed attempts. Locked out for ${minutesLeft} minutes.` });
  }
  next();
};
...
const parentAuth = (req, res, next) => {
  ...
  if (!token || !activeParentSessions[token] || Date.now() > activeParentSessions[token].expiresAt) {
    return res.status(401).json({ error: 'Unauthorized. Parent Command session expired or invalid.' });
  }
...
// Get all kids
app.get('/api/kids', asyncHandler(async (req, res) => {
  const kids = await dbManager.all('SELECT id, name, avatar, points, color_theme FROM kids ORDER BY name ASC');
  res.json(kids);
}));

// Get a single kid's details (for parent editing)
app.get('/api/kids/:id', parentAuth, asyncHandler(async (req, res) => {
  const { id } = req.params;
  const kid = await dbManager.get('SELECT id, name, avatar, points, color_theme FROM kids WHERE id = ?', [id]);
  if (!kid) {
...
// Add a new kid
app.post('/api/kids', asyncHandler(async (req, res) => {
  ...
  const newKid = await dbManager.get('SELECT id, name, avatar, points, color_theme FROM kids WHERE id = ?', [result.id]);
  res.status(201).json(newKid);
}));

// Update a kid's details
app.put('/api/kids/:id', parentAuth, asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  const updatedKid = await dbManager.get('SELECT id, name, avatar, points, color_theme FROM kids WHERE id = ?', [id]);
  res.json(updatedKid);
}));

// Verify a kid's PIN for access
app.post('/api/kids/:id/verify-pin', rateLimiter, asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { pin } = req.body;
...
app.post('/api/verify-parent-pin', rateLimiter, asyncHandler(async (req, res) => {
  const { pin } = req.body;
...
// Delete a kid
app.delete('/api/kids/:id', parentAuth, asyncHandler(async (req, res) => {
  const { id } = req.params;
...
app.get('/api/chores', asyncHandler(async (req, res) => {
...
// Create a chore
app.post('/api/chores', parentAuth, asyncHandler(async (req, res) => {
  ...
  const result = await dbManager.run(
    ...
    [title, description || '', points, schedule_type || 'daily', schedule_days || null, assigned_to || null]
  );
...
// Edit a chore
app.put('/api/chores/:id', parentAuth, asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { title, description, points, schedule_type, schedule_days, assigned_to, is_active } = req.body;
  ...
  await dbManager.run(
    ...
    [updatedTitle, updatedDesc, updatedPoints, updatedType, updatedDays, updatedAssigned, updatedActive, id]
  );
...
app.delete('/api/chores/:id', parentAuth, asyncHandler(async (req, res) => {
  const { id } = req.params;
...
app.get('/api/chores/daily/:kidId/:date', asyncHandler(async (req, res) => {
...
app.post('/api/completions', asyncHandler(async (req, res) => {
  ...
  if (existing) {
    ...
    if (existing.status === 'approved') {
      return res.status(400).json({ error: 'Chore already completed and approved for this day.' });
    }
    ...
    const updated = await dbManager.get('SELECT * FROM chore_completions WHERE id = ?', [existing.id]);
  ...
  const newCompletion = await dbManager.get('SELECT * FROM chore_completions WHERE id = ?', [result.id]);
  res.status(201).json(newCompletion);
}));

// Get all pending completions for Parents' Review Queue
app.get('/api/completions/pending', parentAuth, asyncHandler(async (req, res) => {
...
app.put('/api/completions/:id/approve', parentAuth, asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  const chore = await dbManager.get('SELECT points FROM chores WHERE id = ?', [completion.chore_id]);
  ...
  await dbManager.transaction(async () => {
    ...
    await dbManager.run('UPDATE kids SET points = ? WHERE id = ?', [newPoints, completion.kid_id]);
  });

  const updatedCompletion = await dbManager.get('SELECT * FROM chore_completions WHERE id = ?', [id]);
  res.json({ success: true, completion: updatedCompletion });
}));

// Reject chore completion (sends feedback back)
app.put('/api/completions/:id/reject', parentAuth, asyncHandler(async (req, res) => {
  const { id } = req.params;
...
app.get('/api/rewards', asyncHandler(async (req, res) => {
  const rewards = await dbManager.all('SELECT * FROM rewards WHERE is_active = 1 ORDER BY points_cost ASC');
  res.json(rewards);
}));

// Create a reward
app.post('/api/rewards', parentAuth, asyncHandler(async (req, res) => {
...
// Edit a reward
app.put('/api/rewards/:id', parentAuth, asyncHandler(async (req, res) => {
  const { id } = req.params;
...
// Delete a reward
app.delete('/api/rewards/:id', parentAuth, asyncHandler(async (req, res) => {
  const { id } = req.params;
...
app.post('/api/redemptions', asyncHandler(async (req, res) => {
  const { reward_id, kid_id } = req.body;
  if (!reward_id || !kid_id) {
    return res.status(400).json({ error: 'Reward ID and Kid ID are required.' });
  }

  const reward = await dbManager.get('SELECT * FROM rewards WHERE id = ? AND is_active = 1', [reward_id]);
  ...
  if (kid.points < reward.points_cost) {
    return res.status(400).json({ error: `Not enough points! You have ${kid.points} coins, but this costs ${reward.points_cost} coins.` });
  }
...
app.get('/api/redemptions/pending', parentAuth, asyncHandler(async (req, res) => {
  const pending = await dbManager.all(`
    SELECT rr.*, r.title as reward_title, r.description as reward_description, r.points_cost as reward_cost,
           k.name as kid_name, k.avatar as kid_avatar, k.color_theme as kid_theme
    FROM reward_redemptions rr
...
app.put('/api/redemptions/:id/fulfill', parentAuth, asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  const updatedRedemption = await dbManager.get('SELECT * FROM reward_redemptions WHERE id = ?', [id]);
...
dbManager.initDatabase()
  .then(() => {
    app.listen(PORT, () => {
      console.log(`ChoreQuest server running on http://localhost:${PORT}`);
